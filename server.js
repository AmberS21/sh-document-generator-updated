...
app.all(['/api/ezekia', '/api/ezekia/*'], (req, res) => {
  ...
  // Preserve query string exactly as-is (don't re-encode through URL constructor)
  const queryString = qIndex >= 0 ? req.originalUrl.slice(qIndex) : '';

  // Parse base URL to get hostname and base path
  const parsedBase = new URL(normalizedBaseUrl);
  const isHttps = parsedBase.protocol === 'https:';
  const lib = isHttps ? https : http;
  // Build full path: basePath + ezekiaPath + queryString (raw, unmodified)
  const basePath = parsedBase.pathname.endsWith('/') ? parsedBase.pathname : parsedBase.pathname + '/';
  const fullPath = basePath + ezekiaPath + queryString;
